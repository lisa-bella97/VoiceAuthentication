...
std::vector<ALubyte> OpenALRecorder::record(float seconds) {
    ...
    while (currentTime < (lastTime + 5) && (err = alcGetError(mDevice)) == ALC_NO_ERROR) {
        ...
        if (count > bufferSize) {
            auto data = static_cast<ALubyte *>(calloc(mFrameSize, static_cast<ALuint>(count)));
            free(buffer);
            buffer = data;
            bufferSize = count;
        }
        ...
        for (auto i = 0; i < mFrameSize * count; ++i) {
            result.push_back(buffer[i]);
        }

        dataSize += mFrameSize * count;
...
void OpenALRecorder::record(float seconds, const std::string &fileName) {
